...
// 		} 
	...
	// Add Charges
	// Take bottle count from previous branch
	
	$branchCode = mysql_fetch_assoc(mysql_query("select branch_code from branches where branch_id='".$branchid."'"));
	
	$bottleCnt = 1;
	$prevBottleCount = mysql_fetch_assoc(mysql_query("select bottle_count from ed_branch_bottle where branch_code='".$branchCode['branch_code']."'"));
    ...
    $curBottleCount = $prevBottleCount['bottle_count'] - $bottleCnt;
	
	mysql_query("update ed_branch_bottle where set bottle_count='".$curBottleCount."' where branch_code='".$branchCode['branch_code']."'");
...
// 	exit;
	...
	foreach($_REQUEST['symtoms'] as $key=>$val){
	    ...
	    if($getPatientCount['bottle_count'] >= 50) {
	        $pcount = $getPatientCount['bottle_count'] - $acount;
	        $addFlowerPatient = mysql_query("update ed_branch_bottle set bottle_count='".$pcount."' where branch_code='".$_SESSION['branch_code']."'");
	    } 
