...
<li><a href="branchFlower.php">Branch Flower</a></li>
			...
			<div class="content">
				...
				<div class="title"><h3>	Monthly Total <?= $row['branch']?></h3></title>
					...
					<?php
						$getallpat = mysql_query("select sum(charges) MonthlyTotal,MONTHNAME(created_on) Month,YEAR(created_on) Year,substring_index(patient_id,'-',1),count(distinct patient_id) new_patient from ed_patient_diagnosis where substring_index(patient_id,'-',1) = '".$row['branch_code']."' and pid not in (SELECT pid
...
WHERE lower(patient_name) LIKE  '%test%' or pid = 1) group by MONTH(created_on),YEAR( created_on ) order by YEAR(created_on) desc,MONTH(created_on) desc");
			...
			?>
				...
				<div class="title"><h3>	Monthly Total Panvel/New Panvel</h3></title>
					...
					<tbody>
					<?php
					$getallpat1 = mysql_query("select * from (select sum(charges) MonthlyTotal,MONTHNAME(created_on) Month,YEAR(created_on) Year,substring(patient_id,1,2),count(distinct patient_id) new_patient from ed_patient_diagnosis where substring(patient_id,1,2) in ('NP','OP') and pid not in (SELECT pid
FROM  patient_details
