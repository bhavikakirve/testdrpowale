...
// 		$getPatientCount = mysql_fetch_assoc(mysql_query("select bottle_count from ed_branch_bottle where branch_code like '".$_SESSION['branch_code']."%'"));
		
// 		if($getPatientCount['bottle_count'] >= 50) {
// 		    $pcount = $getPatientCount['bottle_count'] - $bottle_count;
// 		    $addFlowerPatient = mysql_query("update ed_branch_bottle set bottle_count='".$pcount."' where branch_code='".$_SESSION['branch_code']."'");
// 		} 
	}	
	...
	if(mysql_num_rows($checkpat)>0){
		while($r = mysql_fetch_array($checkpat)){
			$pid = $r['pid'];
			$addpat = mysql_query("update  patient_details set patient_name = '".$patient_name."', gender = '".$gender."' ,age ='".$age."' ,place = '".$place."',contact_no ='".$contact_no."', reference = '".$reference."', chief_complain = '".$ccomplain."', family_history = '".$family_history."' where pid = $pid");
		
		}		
	}else{
	    $addpat = mysql_query("insert into patient_details(patient_id,patient_name,gender,age,place,contact_no,reference,chief_complain,family_history,created_by,created_on) values
		('".$patient_id."','".$patient_name."','".$gender."','".$age."','".$place."','".$contact_no."','".$reference."','".$ccomplain."','".$family_history."','".$createdBy."',now())");
		$pid = mysql_insert_id();
	}
...
// 	echo "<pre>";
// 	print_r($_REQUEST['symtoms']);
// 	exit;
	
   
	foreach($_REQUEST['symtoms'] as $key=>$val){
	    
	    if (preg_match("/Acute/i", $val) || $charges == "300") {
	        $acount = 1;
	        $patientCount = 1;
	    } else if($charges == "free") {
	        $acount = 2;
	        $patientCount = 1;
	    } else if($charges == "600") {
	        $acount = 2;
	        $patientCount = 1;
	    } else if($charges == "1200") {
	        $acount = 4;
	        $patientCount = 2;
	    } else if($charges == "1800") {
	        $patientCount = 3;
	        $acount = 6;
	    } else if($charges == "2400") {
	        $patientCount = 4;
	        $acount = 8;
	    } else if($charges == "3000") {
	        $patientCount = 5;
	        $acount = 10;
	    } else {
	        $patientCount = 6;
	    ...
	    //if($getPatientCount['bottle_count'] >= 50) {
	        $bottleCount = $getPatientCount['bottle_count'] - $acount;
	        $addFlowerPatient = mysql_query("update ed_branch_bottle set bottle_count='".$bottleCount."' where branch_code='".$_SESSION['branch_code']."'");
	    //} 
		...
		if(!empty($flower)) {
		    $flowerExplode = explode('&',$flower);
		       ...
		       if($count == 1) {
    		    ...
    		    $relationFlowerName = mysql_fetch_assoc(mysql_query("select relation_flower_name from ed_flower_relation where flower_name=".$flowerName));
    		    
    		    
    		    //this is added for servver 
    		    if($getPatientCount['patient_count'] < 25) {
    		        $pcount = $getPatientCount['patient_count'] + $patientCount;
    		        ...
    		        if(!empty($relationFlowerName)) {
    		            $addRelationFlower = mysql_query("update ed_branch_flower set patient_count='".$pcount."' where flower_name='".$relationFlowerName['relation_flower_name']."' and branch_code='".$_SESSION['branch_code']."'");
    		        }
    		    } 
		}
